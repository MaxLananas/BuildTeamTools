...
public class TreeHeightMenu extends NameListMenu {
    ...
    private static @NonNull List<MutablePair<ItemStack, String>> getTreeHeights() {
        ...
        treeHeights.add(MutablePair.of(new Item(Objects.requireNonNull(XMaterial.LIME_CONCRETE.parseItem())).setDisplayName(
                "Any").build(), "Any"));

        GeneratorModule.getInstance().getTree().getHeights().forEach(h ->
                treeHeights.add(MutablePair.of(Item.create(Objects.requireNonNull(XMaterial.PAPER.get()),
                        StringUtils.capitalize(h)), h)));
