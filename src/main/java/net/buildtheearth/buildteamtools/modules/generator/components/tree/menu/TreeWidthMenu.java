...
public class TreeWidthMenu extends NameListMenu {
    ...
    private static @NonNull List<MutablePair<ItemStack, String>> getTreeWidths() {
        ...
        treeWidths.add(new MutablePair<>(new Item(Objects.requireNonNull(XMaterial.LIME_CONCRETE.parseItem())).setDisplayName(
                "Any").build(), "Any"));

        for (TreeWidth treeWidth : TreeWidth.values())
            treeWidths.add(new MutablePair<>(Item.create(Objects.requireNonNull(XMaterial.JUNGLE_SAPLING.get()),
                    StringUtils.capitalize(treeWidth.getName())), treeWidth.getName()));
