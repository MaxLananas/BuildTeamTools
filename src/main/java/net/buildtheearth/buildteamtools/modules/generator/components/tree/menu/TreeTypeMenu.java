...
public class TreeTypeMenu extends NameListMenu {
    ...
    private static @NonNull List<MutablePair<ItemStack, String>> getTreeTypes() {
        ...
        treeTypes.add(new MutablePair<>(new Item(Objects.requireNonNull(XMaterial.LIME_CONCRETE.parseItem())).setDisplayName(
                "Any").build(), "Any"));

        for (TreeType treeType : TreeType.values())
            treeTypes.add(new MutablePair<>(Item.create(Objects.requireNonNull(XMaterial.OAK_SAPLING.get()),
                    StringUtils.capitalize(treeType.getName())), treeType.getName()));
