...
public class WarpGroupEditMenu extends AbstractMenu {
    ...
    protected void setMenuItemsAsync() {
        ...
        getMenu().getSlot(DESCRIPTION_SLOT).setItem(Item.create(Objects.requireNonNull(XMaterial.BOOK.get()), "§6§lChange " +
                "Description", warpGroup.getDescriptionLore()));

        // Set the Slot item
        int slot = warpGroup.getSlot();
        ArrayList<String> slotLore = ListUtil.createList("", "§eSlot: ", slot < 0 || slot >= 27 ? "§7Auto" : String.valueOf(slot));
        getMenu().getSlot(SLOT_SLOT).setItem(new Item(Objects.requireNonNull(XMaterial.ITEM_FRAME.get())).setDisplayName("§6" +
                "§lChange Slot").setLore(slotLore).build());
        ...
        // Set the delete item
        getMenu().getSlot(DELETE_SLOT).setItem(Item.create(Objects.requireNonNull(XMaterial.BARRIER.get()), "§c§lDelete Warp " +
                "Group", ListUtil.createList("", "§8Click to delete the warp group.")));
    }
