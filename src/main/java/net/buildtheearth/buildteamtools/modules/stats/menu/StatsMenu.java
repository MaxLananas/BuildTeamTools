...
public class StatsMenu extends AbstractMenu {
    ...
    private static final ItemStack GLOBAL_HEAD = HeadFactory.head(HeadTexture.EARTH, "§eGlobal Statistics",
            ListUtil.createList("error"));
    private static final ItemStack ACHIEVEMENTS_HEAD = HeadFactory.head(HeadTexture.GOLDEN_CUP, "§eAchievements",
            ListUtil.createList("error"));
    private final ItemStack PLAYER_HEAD = Item.createPlayerHead("&ePersonal Statistics", getMenuPlayer().getName());
    private final ItemStack TEAM_HEAD = Item.create(Objects.requireNonNull(XMaterial.STONE.get()), "Placeholder",
            ListUtil.createList("placeholder")); //TODO GET HEAD BASED ON COUNTRY/TEAM
